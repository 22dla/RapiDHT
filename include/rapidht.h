...
private:
	...
	*/
	 ...
	 */
	void RealFFT1D(double* vector, const Direction& direction = Direction::X);

	void series1d(double* image, const Direction& direction);

	static void bit_reverse(std::vector<size_t>* indices);
	static void initialize_kernel_host(std::vector<double>* kernel, const int height);
